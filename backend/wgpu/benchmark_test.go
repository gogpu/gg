...
// BenchmarkTessellation benchmarks path tessellation for various path types.
func BenchmarkTessellation(b *testing.B) {
	benchmarks := []struct {
		name string
		path *scene.Path
	}{
		{"rect_100", scene.NewPath().Rectangle(0, 0, 100, 100)},
		{"rect_1000", scene.NewPath().Rectangle(0, 0, 1000, 1000)},
		{"circle_r50", scene.NewPath().Circle(50, 50, 50)},
		{"circle_r100", scene.NewPath().Circle(100, 100, 100)},
		{"triangle", createBenchTriangle()},
		{"complex_20_curves", createComplexPath(20)},
		{"complex_100_curves", createComplexPath(100)},
	}
	...
	for _, bm := range benchmarks {
		b.Run(bm.name, func(b *testing.B) {
			...
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				_ = tess.TessellatePath(bm.path, transform)
			}
		})
	}
}

// createBenchTriangle creates a triangle path for benchmarking.
func createBenchTriangle() *scene.Path {
	path := scene.NewPath()
	path.MoveTo(50, 0)
	path.LineTo(100, 100)
	path.LineTo(0, 100)
...
// createComplexPath creates a path with many curves for benchmarking.
func createComplexPath(curves int) *scene.Path {
	path := scene.NewPath()
	path.MoveTo(0, 50)
...
// BenchmarkTessellationWithTransform benchmarks tessellation with various transforms.
func BenchmarkTessellationWithTransform(b *testing.B) {
	...
	transforms := []struct {
		...
		{"identity", scene.IdentityAffine()},
		{"translate", scene.TranslateAffine(100, 100)},
		{"scale", scene.ScaleAffine(2.0, 2.0)},
		{"rotate", scene.RotateAffine(0.785)}, // 45 degrees
		{"combined", scene.TranslateAffine(50, 50).Multiply(scene.ScaleAffine(1.5, 1.5))},
	}

	for _, tt := range transforms {
		b.Run(tt.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				_ = tess.TessellatePath(path, tt.transform)
			}
		})
	}
}
...
// =============================================================================
// Strip Packing Benchmarks
// =============================================================================

// BenchmarkStripPacking benchmarks GPU data packing performance.
func BenchmarkStripPacking(b *testing.B) {
	sizes := []struct {
		name       string
		stripCount int
		avgWidth   int
		...
		{"10_strips_w50", 10, 50},
		{"100_strips_w50", 100, 50},
		{"500_strips_w100", 500, 100},
		{"1000_strips_w100", 1000, 100},
	}

	for _, sz := range sizes {
		b.Run(sz.name, func(b *testing.B) {
			sb := createBenchStripBuffer(sz.stripCount, sz.avgWidth)
...
// createBenchStripBuffer creates a strip buffer for benchmarking.
func createBenchStripBuffer(stripCount, avgWidth int) *StripBuffer {
	sb := NewStripBuffer()
	for i := 0; i < stripCount; i++ {
		coverage := make([]uint8, avgWidth)
...
// BenchmarkStripPackingInPlace benchmarks in-place packing.
func BenchmarkStripPackingInPlace(b *testing.B) {
	sb := createBenchStripBuffer(500, 100)

	// Pre-allocate buffers
	headers := make([]GPUStripHeader, 1000)
	coverage := make([]uint8, 100000)

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = sb.PackForGPUInto(headers, coverage)
	}
}

// BenchmarkStripMerge benchmarks adjacent strip merging.
func BenchmarkStripMerge(b *testing.B) {
	sizes := []struct {
		name       string
		stripCount int
	}{
	...
	for _, sz := range sizes {
		b.Run(sz.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				b.StopTimer()
				sb := createMergeableStripBuffer(sz.stripCount)
				b.StartTimer()
				sb.MergeAdjacent()
			}
		})
...
// createMergeableStripBuffer creates strips that can be merged.
func createMergeableStripBuffer(count int) *StripBuffer {
	sb := NewStripBuffer()
	for i := 0; i < count; i++ {
		y := i / 10
		x := (i % 10) * 10

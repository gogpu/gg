...
// init registers the wgpu backend on package import.
func init() {
...
// WGPUBackend is a GPU-accelerated rendering backend using gogpu/wgpu.
...
// NewWGPUBackend creates a new WGPU rendering backend.
...
func (b *WGPUBackend) Init() error {
	...
	b.initialized = true
	log.Println("wgpu: backend initialized successfully")
...
func (b *WGPUBackend) Close() {
	...
	log.Println("wgpu: backend closed")
}
...
func (b *WGPUBackend) NewRenderer(width, height int) gg.Renderer {
	...
	if !b.initialized {
		log.Println("wgpu: warning - creating renderer on uninitialized backend")
		return nil
	}

	if width <= 0 || height <= 0 {
		log.Printf("wgpu: warning - invalid dimensions: %dx%d", width, height)
		return nil
	}
...
func (b *WGPUBackend) RenderScene(target *gg.Pixmap, s *scene.Scene) error {
	...
	if err := renderer.RenderToPixmap(target, s); err != nil {
		...
		if errors.Is(err, ErrTextureReadbackNotSupported) {
			...
			log.Printf("wgpu: RenderScene completed on GPU (readback pending wgpu support)")
			return nil
		}

...
// TestTessellationIntegration tests path to strips pipeline.
func TestTessellationIntegration(t *testing.T) {
	tess := NewTessellator()
	...
	tests := []struct {
		...
		{
			name:     "triangle",
			path:     createTrianglePath(50, 10, 90, 90, 10, 90),
			minCount: 50,
			maxCount: 100,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			strips := tess.TessellatePath(tt.path, scene.IdentityAffine())

			if strips.IsEmpty() {
				t.Error("Tessellation produced no strips")
				return
			}

			count := strips.StripCount()
			if count < tt.minCount || count > tt.maxCount {
				t.Errorf("Strip count %d outside expected range [%d, %d]",
					count, tt.minCount, tt.maxCount)
			}

			// Verify GPU packing works
			headers, coverage := strips.PackForGPU()
			if len(headers) != count {
				t.Errorf("Header count %d != strip count %d", len(headers), count)
			}
			if len(coverage) == 0 {
				t.Error("No coverage data")
			}
		})
	}
}

// createTrianglePath creates a triangle path from three vertices.
func createTrianglePath(x1, y1, x2, y2, x3, y3 float32) *scene.Path {
	path := scene.NewPath()
	path.MoveTo(x1, y1)
	path.LineTo(x2, y2)
	path.LineTo(x3, y3)
...
// TestTessellationWithTransform tests transformed path tessellation.
func TestTessellationWithTransform(t *testing.T) {
	tess := NewTessellator()
	path := scene.NewPath().Rectangle(0, 0, 50, 50)
	...
	tests := []struct {
		...
		{"scale", scene.ScaleAffine(2, 2)},
		{"rotate", scene.RotateAffine(0.5)},
		{"combined", scene.TranslateAffine(50, 50).Multiply(scene.ScaleAffine(1.5, 1.5))},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			strips := tess.TessellatePath(path, tt.transform)
			if strips.IsEmpty() {
				t.Error("Tessellation produced no strips")
			}
		})
...
// TestTessellationFillRules tests different fill rules produce correct results.
func TestTessellationFillRules(t *testing.T) {
	// Create path with self-intersection (concentric rectangles)
	path := scene.NewPath()
	path.Rectangle(0, 0, 100, 100)
	path.Rectangle(25, 25, 50, 50)

	rules := []scene.FillStyle{
		scene.FillNonZero,
		scene.FillEvenOdd,
	}

	ruleNames := []string{"NonZero", "EvenOdd"}
	for i, rule := range rules {
		t.Run(ruleNames[i], func(t *testing.T) {
			tess := NewTessellator()
			tess.SetFillRule(rule)
			...
			if strips.FillRule() != rule {
				t.Errorf("Fill rule mismatch: got %v, want %v",
					strips.FillRule(), rule)
			}
		})
	}
}
...
func TestBackendRegistration(t *testing.T) {
	...
	if b == nil {
		t.Fatal("backend.Get(BackendWGPU) returned nil")
	}

	// Verify name
	if b.Name() != backend.BackendWGPU {
		t.Errorf("Name() = %q, want %q", b.Name(), backend.BackendWGPU)
	}
}

...
func (a *NativeSDFAccelerator) Name() string { return "sdf-gpu-native" }
...
func (a *NativeSDFAccelerator) Init() error {
	...
	if err := a.initGPU(); err != nil {
		log.Printf("native-sdf: GPU init failed, using CPU fallback: %v", err)
	}
	return nil
}
...
func (a *NativeSDFAccelerator) initGPU() error {
	...
	a.gpuReady = true
	log.Printf("native-sdf: GPU accelerator initialized (%s)", selected.Info.Name)
	return nil
}

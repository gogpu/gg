...
func (s *GPUShared) SetDeviceProvider(provider gpucontext.DeviceProvider) error {
	...
	if s.strategy == strategyRasterAtlas {
		// Software adapter: skip SDF pipeline creation (hangs on SPIR-V interpreter).
		// Device stays alive for texture operations (ADR-046, BUG-SW-002).
		slogger().Info("gpu-shared: rasterAtlas strategy — SDF pipelines skipped",

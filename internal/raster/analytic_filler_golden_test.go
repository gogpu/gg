...
func TestAnalyticFiller_StarBlitY9(t *testing.T) {
	...
	topXL := int32(3276800) + skFixedMul(-20480, 98304)
	botXL := int32(3276800) + skFixedMul(-20480, 163840) // y=10 - UpperY
	// Edge0 (right, w=+1): UpperX=3276800, PixelDX=+20480
	topXR := int32(3276800) + skFixedMul(20480, 98304)
	botXR := int32(3276800) + skFixedMul(20480, 163840)
	t.Logf("\nFrom-origin values: ul=%d ur=%d ll=%d lr=%d", topXL, topXR, botXL, botXR)
	t.Logf("  C++ values:       ul=%d ur=%d ll=%d lr=%d", 3246080, 3307520, 3225600, 3328000)

	for i := range af.coverage {
		af.coverage[i] = 0
	}
	af.blitTrapezoidRow(topXL, topXR, botXL, botXR, 209715, 209715, 255)

...
| **SVG** | Full SVG renderer (`gg/svg`): parse + render SVG XML with color override for theming, **retained scene lowering** (`RenderToScene` — resolution-independent vector icons via scene graph), SVG path data parser (`ParseSVGPath`), transform-aware `FillPath`/`StrokePath` |

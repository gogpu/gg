...
//
// The main type is ShapingCache, which caches ShapedRun results using a sharded
// map architecture with LRU eviction. The 16-shard design reduces lock contention
...
//
// The LRU update on hit requires a write lock, which adds latency compared to
// a pure read-only cache. For hot paths where LRU accuracy is less critical,
// consider using GetOrCreate with a fast create function.
//
// This package is designed for GPU text rendering pipelines where caching shaped
// glyphs is critical for performance.
package cache
